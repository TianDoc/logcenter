...
def sendmail2(host,message,time,question,name):                         ###发送邮件函数
    if question.strip()!='up' and question.strip()!='down':
        ...
        for i in getemailaddress(name):
            os.system('sh /usr/local/logtest/mailsend.sh %s%s%s' %(i['emailaddress'],subject,message))
    else:
...
def screen():                  ###读取redis信息并进行判断
    ...
    while True:
        ...
        if time.time()-time_start>60:
            question=getcontrol()  ###每隔1分钟重新加载一次塞选条件
            time_start=time.time()
        try:
            if count >2000:
                save2()
...
if __name__ == '__main__': 
     ...
     try:
         ...
         sendmail2("1","traceback.print_exc():"+traceback.print_exc(),"1","1","测试组")
